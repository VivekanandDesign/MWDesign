...
export function IconDetailsModal({
  isOpen,
  iconName,
  activeTab,
  onClose,
  ...
  const iconCategory = Object.entries(iconData.categories).find(([_, cat]) => 
  ...
  const currentIconIndex = iconCategory?.icons.indexOf(iconName || '') || 0;
  const canNavigatePrev = currentIconIndex > 0;
  const canNavigateNext = currentIconIndex < (iconCategory?.icons.length || 0) - 1;
  ...
  if (!isOpen || !iconName) return null;
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              className="p-2"
              title={isFavorite(iconName) ? 'Remove from favorites' : 'Add to favorites'}
            >
              <Heart 
                className={`w-4 h-4 ${
                  isFavorite(iconName) 
        ...
        <div className="flex flex-col lg:flex-row max-h-[calc(90vh-80px)]">
            ...
            <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-medium">{iconCategory?.name || 'Unknown'}</div>
                </div>
                <div>
          ...
          <div className="flex-1 flex flex-col max-h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {activeTab === 'customize' && (
                <IconCustomizationControls iconName={iconName} />
              )}
              
              {activeTab === 'code' && (
                <IconCodeSnippets iconName={iconName} />
              )}
            </div>
          </div>
        </div>
