...
export const MWLoader = forwardRef<HTMLDivElement, MWLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'primary', 
      ...
      return (
        ...
        >
          <span className={clsx(
            ...
            'before:content-["MW"] before:absolute before:left-0 before:top-0',
            'before:w-0 before:animate-[typewriter_2s_steps(2,end)_infinite]',
            'before:border-r-2 before:border-r-mw-blue-600',
            'before:bg-white before:dark:bg-mw-gray-900'
          )}>
            MW
          </span>
        </div>
    ...
    return (
      <div
        ref={ref}
        className={clsx(
          'inline-flex items-center justify-center font-bold tracking-wider select-none',
          loaderSizes[size],
          loaderVariants[variant],
          getAnimationClass(),
          className
...
// Enhanced MW Loader with dots animation
export interface MWDotsLoaderProps extends Omit<MWLoaderProps, 'animation'> {
  showDots?: boolean
  dotAnimation?: 'typing' | 'bouncing' | 'fading'
}

export const MWDotsLoader = forwardRef<HTMLDivElement, MWDotsLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'primary',
        ...
        {showDots && (
          <div className="flex items-center space-x-1 ml-2">
            <div className={clsx(
              ...
              variant === 'primary' ? 'bg-mw-blue-600' : 'bg-current',
...
// Brand Logo Loader (if you have an MW logo)
...
export const MWBrandLoader = forwardRef<HTMLDivElement, MWBrandLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'pulse',
    ...
    return (
      ...
      >
        {/* MW Logo Container */}
        <div className={clsx(
          'flex items-center justify-center rounded-lg bg-mw-blue-600 text-white font-bold',
          brandSizes[size],
          getAnimationClass()
        )}>
          MW
        ...
        {/* Optional Text */}
        {showText && (
          <div className="text-sm text-mw-gray-600 dark:text-mw-gray-400 font-medium">
            Loading...
          </div>
        )}
      </div>
...
// Progressive MW Text Loader - text loads from left to right
...
export const MWProgressiveLoader = forwardRef<HTMLDivElement, MWProgressiveLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'primary',
    ...
    return (
      <div
        ref={ref}
        className={clsx(
          'inline-flex items-center justify-center font-bold tracking-wider relative overflow-hidden',
          loaderSizes[size],
          className
        )}
        role="status"
...
export const MWGlitchLoader = forwardRef<HTMLDivElement, MWGlitchLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    intensity = 'medium',
    ...
    return (
      <div
        ref={ref}
        className={clsx(
          'inline-flex items-center justify-center font-bold tracking-wider relative',
          loaderSizes[size],
          className
        )}
        style={{
...
export const MWHeartbeatLoader = forwardRef<HTMLDivElement, MWHeartbeatLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'primary',
...
// Matrix/Rain Effect MW Loader
export interface MWMatrixLoaderProps extends React.HTMLAttributes<HTMLDivElement> {
  size?: 'sm' | 'md' | 'lg' | 'xl'
  speed?: 'slow' | 'normal' | 'fast'
}

export const MWMatrixLoader = forwardRef<HTMLDivElement, MWMatrixLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    speed = 'normal',
...
export const MWNeonLoader = forwardRef<HTMLDivElement, MWNeonLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    color = 'blue',
...
export const MWBounceLoader = forwardRef<HTMLDivElement, MWBounceLoaderProps>(
  ({ 
    className, 
    size = 'md', 
    variant = 'primary',
    ...
    const getVariantClass = (letter: 'M' | 'W') => {
      switch (variant) {
        case 'primary':
          return 'text-mw-blue-600 dark:text-mw-blue-400'
        case 'gradient':
