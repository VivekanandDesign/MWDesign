export function generateIconCode(iconName, format, customization) {
...
function generateSVGCode(iconName, customization) {
    const { size, color, strokeWidth, fillColor } = customization;
...
function generateJSXCode(iconName, customization) {
    const { size, color, strokeWidth, fillColor } = customization;
    const componentName = iconName.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
...
function generateVueCode(iconName, customization) {
    const { size, color, strokeWidth, fillColor } = customization;
    const componentName = iconName.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
    return `<template>
  <svg 
...
function generateSvelteCode(iconName, customization) {
    const { size, color, strokeWidth, fillColor } = customization;
    return `<svg 
  width={${size}} 
  height={${size}} 
...
export function generateCodeSnippets(iconName, customization) {
    return {
...
//# sourceMappingURL=iconCodeGenerator.js.map