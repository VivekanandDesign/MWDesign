...
export function IconCodeSnippets({ iconName, className = '' }) {
    ...
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="space-y-4">
        {codeBlocks.map((block) => (<div key={block.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={() => handleDownload(block.filename, block.content)} variant="ghost" size="sm" className="text-xs">
                  <Download className="w-3 h-3 mr-1"/>
                  Download
                </Button>
                
                <Button onClick={() => handleCopy(block.id, block.content)} variant="ghost" size="sm" className="text-xs" disabled={copiedState[block.id]}>
            ...
            <div className="p-0 max-h-96 overflow-hidden">
              <CodeSnippet code={block.content} language={block.language}/>
            </div>
          </div>))}
      </div>
...
//# sourceMappingURL=IconCodeSnippets.jsx.map