...
export const CopyIconButton = ({ iconName, variant = 'primary', size = 'sm', className = '', showDropdown = true, iconSize = 24, strokeWidth = 2 }) => {
    ...
    if (!showDropdown) {
        ...
        return (<Button size={size} variant={variant} onClick={() => handleCopy(CopyType.SVG)} disabled={isLoading} className={`${className} ${isIconCopied ? 'bg-green-600 hover:bg-green-700' : ''}`} title="Copy SVG code">
        {getButtonIcon()}
    ...
    return (<div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <div className="flex">
        {/* Main Copy Button */}
        <Button size={size} variant={variant} onClick={() => handleCopy(CopyType.SVG)} disabled={isLoading} className={`rounded-r-none border-r-0 ${isIconCopied ? 'bg-green-600 hover:bg-green-700 border-green-600' : ''}`} title="Copy SVG code">
          {getButtonIcon()}
          {size !== 'sm' && <span className="ml-1">{getButtonText()}</span>}
        </Button>

        {/* Dropdown Toggle */}
        <Button size={size} variant={variant} onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`rounded-l-none px-2 ${isIconCopied ? 'bg-green-600 hover:bg-green-700 border-green-600' : ''}`} title="More copy options">
      ...
      {isDropdownOpen && (<>
          ...
          <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-mw-gray-800 border border-mw-gray-200 dark:border-mw-gray-700 rounded-lg shadow-lg z-50">
            <div className="py-1">
              {copyOptions.map((option) => (<button key={option.type} onClick={() => handleCopy(option.type)} disabled={isLoading} className="w-full px-4 py-3 text-left hover:bg-mw-gray-50 dark:hover:bg-mw-gray-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-start gap-3 transition-colors">
      ...
      {/* Error Toast */}
      {copyState.error && (<div className="absolute top-full left-0 mt-2 w-64 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 shadow-lg z-50">
...
export const SimpleCopyButton = ({ iconName, copyType = CopyType.SVG, size = 16, className = '' }) => {
    ...
    return (<button onClick={() => copyIcon(iconName, copyType, { size })} className={`inline-flex items-center justify-center w-6 h-6 rounded hover:bg-mw-gray-100 dark:hover:bg-mw-gray-700 transition-colors ${className}`} title={`Copy ${copyType.toUpperCase()}`}>
      {isIconCopied ? (<Check className="w-3 h-3 text-green-600"/>) : (<Copy className="w-3 h-3 text-mw-gray-600 dark:text-mw-gray-400"/>)}
    </button>);
};
//# sourceMappingURL=CopyIconButton.jsx.map