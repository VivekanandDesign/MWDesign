...
export function KeyboardShortcutsHelp({ isOpen, onClose, className = '' }) {
    ...
    return (<div className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 ${className}`}>
        ...
        <div className="p-6 space-y-6">
          ...
          {shortcutGroups.length > 0 && (<>
              ...
              <div>
                ...
                {shortcutGroups.map((group) => (<div key={group.name} className="mb-4">
                    <h4 className="font-medium mb-2">{group.name}</h4>
                    <div className="grid gap-2">
                      {group.shortcuts.map((shortcut, index) => (<div key={index} className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/20 rounded">
                          <span className="text-sm">{shortcut.description}</span>
                          <div className="flex items-center gap-1">
                            {formatShortcut(shortcut).map((part, partIndex) => (<Badge key={partIndex} variant="outline" className="text-xs">
                                {part}
                              </Badge>))}
                          </div>
                        </div>))}
...
//# sourceMappingURL=KeyboardShortcutsHelp.jsx.map