...
export function IconRelatedSuggestions({ iconName, relatedIcons, onIconSelect, className = '' }) {
    ...
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Search related icons..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600"/>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {filterOptions.map((filter) => {
            ...
            return (<button key={filter.id} onClick={() => setSelectedFilter(filter.id)} className={`
      ...
      {currentCategory && selectedFilter === 'category' && (<div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
      ...
      {/* Icon Grid */}
      {filteredSuggestions.length > 0 ? (<div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
          {filteredSuggestions.map((suggestionIcon) => (<div key={suggestionIcon} className="group relative aspect-square border border-gray-200 dark:border-gray-700 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-all duration-200 hover:scale-105" onClick={() => onIconSelect === null || onIconSelect === void 0 ? void 0 : onIconSelect(suggestionIcon)}>
              {/* Icon */}
              <div className="flex items-center justify-center h-full">
                <Icon icon={Star} size="md" className="text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200"/>
              </div>
              ...
              <button onClick={(e) => {
                ...
                <Heart className={`w-3 h-3 ${isFavorite(suggestionIcon)
          ...
          {searchQuery && (<Button onClick={() => setSearchQuery('')} variant="ghost" size="sm" className="mt-3">
              Clear search
            </Button>)}
        </div>)}
      ...
      <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => setSelectedFilter('category')} variant="ghost" size="sm" className="text-xs">
            More from {(currentCategory === null || currentCategory === void 0 ? void 0 : currentCategory.name) || 'this category'}
          </Button>
          <Button onClick={() => setSelectedFilter('popular')} variant="ghost" size="sm" className="text-xs">
            Popular icons
          </Button>
          <Button onClick={() => setSelectedFilter('recent')} variant="ghost" size="sm" className="text-xs">
            Recently used
          </Button>
        </div>
      </div>
...
//# sourceMappingURL=IconRelatedSuggestions.jsx.map