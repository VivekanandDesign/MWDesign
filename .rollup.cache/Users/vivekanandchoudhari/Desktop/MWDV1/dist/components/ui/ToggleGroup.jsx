...
export function ToggleGroup(_a) {
    ...
    return (<ToggleGroupContext.Provider value={{
      ...
      <div className={clsx('inline-flex items-center justify-center rounded-md', className)} role="group" {...props}>
        {children}
      </div>
...
export function ToggleGroupItem(_a) {
    ...
    return (<button onClick={handleClick} disabled={isDisabled} data-state={isSelected ? 'on' : 'off'} className={clsx('inline-flex items-center justify-center whitespace-nowrap rounded-md font-medium transition-colors', 'focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-mw-blue-500', 'disabled:pointer-events-none disabled:opacity-50', 'first:rounded-l-md last:rounded-r-md [&:not(:first-child):not(:last-child)]:rounded-none', toggleSizes[context.size], isSelected ? variantConfig.selected : variantConfig.base, className)} {...props}>
      {children}
    </button>);
}
...
//# sourceMappingURL=ToggleGroup.jsx.map