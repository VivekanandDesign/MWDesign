...
function TimeInput({ value, onChange, min, max, step = 1, disabled, className }) {
    ...
    return (<div className={cn('relative', className)}>
      <input ref={inputRef} type="text" value={inputValue} onChange={handleInputChange} onBlur={handleInputBlur} onFocus={handleInputFocus} onKeyDown={handleKeyDown} disabled={disabled} className="w-12 text-center bg-transparent border-none outline-none text-mw-gray-900 dark:text-white disabled:opacity-50" maxLength={2}/>
      
      <div className="absolute right-0 top-0 h-full flex flex-col">
        <button type="button" onClick={handleIncrement} disabled={disabled} className="flex-1 px-1 text-mw-gray-400 hover:text-mw-gray-600 dark:hover:text-mw-gray-200 disabled:opacity-50">
          <ChevronUp className="w-3 h-3"/>
        </button>
        <button type="button" onClick={handleDecrement} disabled={disabled} className="flex-1 px-1 text-mw-gray-400 hover:text-mw-gray-600 dark:hover:text-mw-gray-200 disabled:opacity-50">
...
export function TimePicker({ value = '', onChange, format = '24h', showSeconds = false, disabled = false, label, error, helpText, placeholder = 'Select time', className, size = 'md', step = 1 }) {
    ...
    return (<div className={className}>
      ...
      <div className="relative" ref={dropdownRef}>
        <button type="button" onClick={() => !disabled && setIsOpen(!isOpen)} disabled={disabled} className={cn('relative w-full pl-3 pr-10 text-left border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-mw-blue-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed', timeInputSizes[size], error
            ? 'border-red-300 dark:border-red-600'
            : 'border-mw-gray-300 dark:border-mw-gray-600', 'bg-white dark:bg-mw-gray-800 text-mw-gray-900 dark:text-white')}>
        ...
        {isOpen && (<div className="absolute z-50 w-full mt-1 bg-white dark:bg-mw-gray-800 border border-mw-gray-200 dark:border-mw-gray-700 rounded-md shadow-lg">
            <div className="p-4">
              <div className="flex items-center justify-center space-x-2">
                {/* Hours */}
                <TimeInput value={displayHours} onChange={handleHoursChange} min={format === '12h' ? 1 : 0} max={format === '12h' ? 12 : 23} disabled={disabled}/>
                
                <span className="text-mw-gray-500">:</span>
                
                {/* Minutes */}
                <TimeInput value={minutes} onChange={handleMinutesChange} min={0} max={59} step={step} disabled={disabled}/>
                
                {showSeconds && (<>
                    <span className="text-mw-gray-500">:</span>
                    <TimeInput value={seconds} onChange={handleSecondsChange} min={0} max={59} disabled={disabled}/>
                  </>)}
                
                {format === '12h' && (<div className="ml-2 flex flex-col space-y-1">
                    <button type="button" onClick={() => handleAmpmChange('AM')} disabled={disabled} className={cn('px-2 py-1 text-xs rounded', ampm === 'AM'
                    ...
                    <button type="button" onClick={() => handleAmpmChange('PM')} disabled={disabled} className={cn('px-2 py-1 text-xs rounded', ampm === 'PM'
                    ? 'bg-mw-blue-100 dark:bg-mw-blue-900/20 text-mw-blue-700 dark:text-mw-blue-300'
                    : 'text-mw-gray-600 dark:text-mw-gray-300 hover:bg-mw-gray-100 dark:hover:bg-mw-gray-700')}>
                      PM
                    </button>
