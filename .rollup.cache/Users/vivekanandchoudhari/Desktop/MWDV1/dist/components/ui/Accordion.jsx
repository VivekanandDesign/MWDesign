...
export function Accordion({ type = 'single', defaultValue, value, onValueChange, children, className }) {
    ...
    return (<AccordionContext.Provider value={{ type, value: currentValue, onValueChange: handleValueChange }}>
...
export function AccordionItem({ value, children, className }) {
    ...
    return (<AccordionItemContext.Provider value={{ value, isOpen, toggle }}>
...
export function AccordionTrigger({ children, className }) {
    ...
    return (<button type="button" onClick={context.toggle} className={clsx('flex w-full items-center justify-between py-4 font-medium transition-all', 'text-left text-mw-gray-900 dark:text-mw-gray-100', 'hover:text-mw-blue-600 dark:hover:text-mw-blue-400', 'focus:outline-none focus:ring-2 focus:ring-mw-blue-500 focus:ring-offset-2 rounded-sm', className)}>
      {children}
      <ChevronDown className={clsx('h-4 w-4 shrink-0 transition-transform duration-200', context.isOpen && 'rotate-180')}/>
    </button>);
}
export function AccordionContent({ children, className }) {
    ...
    return (<div className={clsx('overflow-hidden transition-all duration-300', context.isOpen ? 'animate-in slide-in-from-top-1' : 'animate-out slide-out-to-top-1 hidden')}>
