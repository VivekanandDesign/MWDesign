...
function TimeSelect({ value, onChange, options, format, placeholder, disabled, minTime, maxTime }) {
    ...
    return (<div ref={selectRef} className="relative">
      ...
      {isOpen && (<div className="absolute z-50 mt-1 w-full bg-white dark:bg-mw-gray-800 border border-mw-gray-200 dark:border-mw-gray-700 rounded-md shadow-lg max-h-60 overflow-hidden">
          <div className="p-2 border-b border-mw-gray-200 dark:border-mw-gray-700">
            <input type="text" placeholder="Search time..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full px-2 py-1 text-sm border border-mw-gray-300 dark:border-mw-gray-600 rounded bg-white dark:bg-mw-gray-900 text-mw-gray-900 dark:text-white placeholder-mw-gray-500 dark:placeholder-mw-gray-400 focus:outline-none focus:ring-1 focus:ring-mw-blue-500"/>
          </div>
          
          <div className="overflow-y-auto max-h-44">
            {filteredOptions.length > 0 ? (filteredOptions.map((option, index) => (<button key={index} onClick={() => {
                    onChange(option);
                    setIsOpen(false);
                    setSearchTerm('');
                }} className={cn('w-full px-3 py-2 text-left text-sm hover:bg-mw-gray-100 dark:hover:bg-mw-gray-700', value === option && 'bg-mw-blue-100 dark:bg-mw-blue-900/20 text-mw-blue-700 dark:text-mw-blue-300')}>
                  {option}
                </button>))) : (<div className="px-3 py-2 text-sm text-mw-gray-500 dark:text-mw-gray-400">
                No times found
              </div>)}
          </div>
        </div>)}
...
export function TimeRangePicker({ value, onChange, placeholder = 'Select time range', disabled = false, required = false, clearable = true, format = '12', minuteStep = 15, minTime, maxTime, className, presets = defaultPresets }) {
    ...
    return (<div ref={containerRef} className={cn('relative', className)}>
      ...
      {!isRangeValid && selectedRange.from && selectedRange.to && (<p className="mt-1 text-xs text-mw-red-600 dark:text-mw-red-400">
            ...
            <div className="p-4 min-w-[320px]">
              <div className="space-y-4">
                <div>
                  ...
                  <TimeSelect value={selectedRange.from} onChange={handleFromTimeChange} options={timeOptions} format={format} placeholder="Select start time" disabled={disabled} minTime={minTime} maxTime={selectedRange.to ? (format === '12' ? convertTo24Hour(selectedRange.to) : selectedRange.to) : maxTime}/>
                </div>
                ...
                <div>
                  ...
                  <TimeSelect value={selectedRange.to} onChange={handleToTimeChange} options={timeOptions} format={format} placeholder="Select end time" disabled={disabled || !selectedRange.from} minTime={selectedRange.from ? (format === '12' ? convertTo24Hour(selectedRange.from) : selectedRange.from) : minTime} maxTime={maxTime}/>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 mt-4 border-t border-mw-gray-200 dark:border-mw-gray-700">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setIsOpen(false)} disabled={!selectedRange.from || !isRangeValid} className="px-3 py-1 text-sm bg-mw-blue-500 text-white rounded hover:bg-mw-blue-600 disabled:opacity-50 disabled:cursor-not-allowed">
                    Apply
                  </button>
                </div>
              </div>
...
//# sourceMappingURL=TimeRangePicker.jsx.map