...
export function Autocomplete({ options, value: controlledValue, onChange, onSearch, placeholder = 'Search or select...', multiple = false, disabled = false, loading = false, label, error, helpText, size = 'md', clearable = true, creatable = false, onCreate, maxSelections, className, noOptionsText = 'No options found', filterOptions = defaultFilterOptions }) {
    ...
    return (<div className={className}>
      {label && (<label className="block text-sm font-medium text-mw-gray-700 dark:text-mw-gray-300 mb-1">
          {label}
        </label>)}
      
      <div className="relative" ref={containerRef}>
        <div className={cn('relative w-full border rounded-md bg-white dark:bg-mw-gray-800 focus-within:ring-2 focus-within:ring-mw-blue-500 focus-within:border-transparent', autocompleteSizes[size], error
            ? 'border-red-300 dark:border-red-600'
            : 'border-mw-gray-300 dark:border-mw-gray-600', disabled && 'opacity-50 cursor-not-allowed')}>
          <div className="flex items-center flex-wrap gap-1 px-3 py-2">
            ...
            return (<span key={val} className="inline-flex items-center gap-1 px-2 py-1 bg-mw-blue-100 dark:bg-mw-blue-900/20 text-mw-blue-700 dark:text-mw-blue-300 text-xs rounded">
                  {(option === null || option === void 0 ? void 0 : option.label) || val}
                  <button type="button" onClick={() => handleRemoveValue(val)} className="text-mw-blue-600 dark:text-mw-blue-400 hover:text-mw-blue-800 dark:hover:text-mw-blue-200">
                    <X className="w-3 h-3"/>
                  </button>
                </span>);
        })}
            ...
            {/* Input */}
            <input ref={inputRef} type="text" value={multiple ? query : displayValue} onChange={handleInputChange} onKeyDown={handleKeyDown} onFocus={() => setIsOpen(true)} placeholder={selectedValues.length > 0 ? '' : placeholder} disabled={disabled} className="flex-1 min-w-0 bg-transparent border-none outline-none text-mw-gray-900 dark:text-white placeholder-mw-gray-500 dark:placeholder-mw-gray-400"/>

            {/* Actions */}
            <div className="flex items-center gap-1">
              {loading && (<div className="animate-spin rounded-full h-4 w-4 border-2 border-mw-gray-300 border-t-mw-blue-600"/>)}
              
              {clearable && (selectedValues.length > 0 || query) && (<button type="button" onClick={handleClear} className="text-mw-gray-400 hover:text-mw-gray-600 dark:hover:text-mw-gray-200">
                  <X className="w-4 h-4"/>
                </button>)}
              
              <button type="button" onClick={() => setIsOpen(!isOpen)} className="text-mw-gray-400 hover:text-mw-gray-600 dark:hover:text-mw-gray-200">
        ...
        {/* Dropdown */}
        {isOpen && (<div className="absolute z-50 w-full mt-1 bg-white dark:bg-mw-gray-800 border border-mw-gray-200 dark:border-mw-gray-700 rounded-md shadow-lg max-h-60 overflow-auto">
            {allOptions.length === 0 ? (<div className="px-3 py-2 text-sm text-mw-gray-500 dark:text-mw-gray-400">
                {noOptionsText}
              </div>) : (<ul ref={listRef} className="py-1">
                {Object.entries(groupedOptions).map(([groupName, groupOptions]) => (<React.Fragment key={groupName}>
                    {groupName !== 'ungrouped' && (<li className="px-3 py-1 text-xs font-semibold text-mw-gray-500 dark:text-mw-gray-400 uppercase tracking-wide">
                        {groupName}
                      </li>)}
                    {groupOptions.map((option, index) => {
                        ...
                        return (<li key={option.value} className={cn('px-3 py-2 text-sm cursor-pointer flex items-center justify-between', isHighlighted && 'bg-mw-blue-100 dark:bg-mw-blue-900/20', normalOption.disabled && 'opacity-50 cursor-not-allowed', isCreate && 'font-medium text-mw-blue-600 dark:text-mw-blue-400')} onClick={() => !normalOption.disabled && handleSelectOption(option)}>
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-mw-gray-900 dark:text-white">
                              {option.label}
                            </div>
                            {option.description && (<div className="text-xs text-mw-gray-500 dark:text-mw-gray-400">
                                {option.description}
                              </div>)}
                          </div>
                          
                          {isSelected && (<Check className="w-4 h-4 text-mw-blue-600 dark:text-mw-blue-400"/>)}
                        </li>);
                    })}
                  </React.Fragment>))}
              </ul>)}
      ...
      {helpText && !error && (<p className="mt-1 text-xs text-mw-gray-500 dark:text-mw-gray-400">
          {helpText}
        </p>)}
