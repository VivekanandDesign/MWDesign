...
function TreeItem({ node, level, isExpanded, isSelected, hasChildren, onToggle, onSelect, showIcons, showLines, isLast = false, parentLines = [] }) {
    ...
    return (<>
      <div className={cn('group flex items-center py-1 px-2 hover:bg-mw-gray-100 dark:hover:bg-mw-gray-800 cursor-pointer transition-colors relative', isSelected && 'bg-mw-blue-100 dark:bg-mw-blue-900/20 text-mw-blue-700 dark:text-mw-blue-300', node.disabled && 'opacity-50 cursor-not-allowed', level > 0 && 'ml-4')} onClick={handleSelect} style={{ paddingLeft: `${level * 20 + 8}px` }}>
        {/* Tree lines */}
        {showLines && level > 0 && (<div className="absolute left-0 top-0 h-full pointer-events-none">
            {parentLines.map((hasLine, index) => (<div key={index} className="absolute top-0 h-full w-px bg-mw-gray-300 dark:bg-mw-gray-600" style={{ left: `${index * 20 + 10}px` }}>
                {hasLine && <div className="w-full h-full bg-mw-gray-300 dark:bg-mw-gray-600"/>}
              </div>))}
            <div className="absolute top-0 h-1/2 w-px bg-mw-gray-300 dark:bg-mw-gray-600" style={{ left: `${level * 20}px` }}/>
            <div className="absolute top-1/2 w-2 h-px bg-mw-gray-300 dark:bg-mw-gray-600" style={{ left: `${level * 20}px` }}/>
            {!isLast && (<div className="absolute top-1/2 bottom-0 w-px bg-mw-gray-300 dark:bg-mw-gray-600" style={{ left: `${level * 20}px` }}/>)}
          </div>)}

        {/* Expand/collapse button */}
        <div className="flex items-center justify-center w-4 h-4 mr-2">
          {hasChildren ? (<button onClick={handleToggle} className="text-mw-gray-500 hover:text-mw-gray-700 dark:hover:text-mw-gray-300">
              {isExpanded ? (<ChevronDown className="w-3 h-3"/>) : (<ChevronRight className="w-3 h-3"/>)}
            </button>) : (showLines && <div className="w-3 h-3"/>)}
        </div>

        {/* Icon */}
        {showIcons && (<div className="flex items-center justify-center w-4 h-4 mr-2 text-mw-gray-500">
      ...
      {/* Children */}
      {hasChildren && isExpanded && node.children && (<div>
          {node.children.map((child, index) => (<TreeItemContainer key={child.id} node={child} level={level + 1} onToggle={onToggle} onSelect={onSelect} showIcons={showIcons} showLines={showLines} isLast={index === node.children.length - 1} parentLines={currentLines}/>))}
        </div>)}
    </>);
}
function TreeItemContainer(_a) {
    ...
    return (<TreeItem {...props} node={node} isExpanded={isExpanded} isSelected={isSelected} hasChildren={hasChildren} onToggle={handleToggle} onSelect={handleSelect}/>);
}
export function TreeView({ data, selectedIds: controlledSelectedIds, expandedIds: controlledExpandedIds, onSelect, onExpand, multiple = false, defaultExpanded = [], defaultSelected = [], showIcons = true, showLines = false, className }) {
    ...
    const renderTreeItem = (node, level, isLast, parentLines = []) => {
        ...
        <TreeItem node={node} level={level} isExpanded={isExpanded} isSelected={isSelected} hasChildren={hasChildren} onToggle={handleToggle} onSelect={handleSelect} showIcons={showIcons} showLines={showLines} isLast={isLast} parentLines={parentLines}/>
        
        {hasChildren && isExpanded && node.children && (<div>
            {node.children.map((child, index) => renderTreeItem(child, level + 1, index === node.children.length - 1, currentLines))}
          </div>)}
      </React.Fragment>);
    };
    return (<div className={cn('py-2', className)}>
      {data.map((node, index) => renderTreeItem(node, 0, index === data.length - 1))}
    </div>);
}
