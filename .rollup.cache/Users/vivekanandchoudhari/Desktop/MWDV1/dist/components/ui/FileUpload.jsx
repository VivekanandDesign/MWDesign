...
function FileItem({ file, onRemove, showPreview = true }) {
    ...
    return (<div className="flex items-center p-3 bg-mw-gray-50 dark:bg-mw-gray-800 rounded-lg border border-mw-gray-200 dark:border-mw-gray-700">
      <div className="flex-shrink-0 mr-3">
        {previewUrl ? (<img src={previewUrl} alt={file.name} className="w-10 h-10 object-cover rounded"/>) : (<div className="w-10 h-10 bg-mw-gray-200 dark:bg-mw-gray-700 rounded flex items-center justify-center text-mw-gray-500">
            {getFileIcon(file)}
          </div>)}
      </div>
      ...
      {onRemove && (<button onClick={onRemove} className="flex-shrink-0 ml-2 p-1 text-mw-gray-400 hover:text-mw-gray-600 dark:hover:text-mw-gray-200">
          <X className="w-4 h-4"/>
        </button>)}
    </div>);
}
function DragDropZone({ onDrop, accept, multiple = false, disabled = false, children, className }) {
    ...
    return (<div onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop} className={cn('relative', isDragOver && 'opacity-75', className)}>
      {children}
    </div>);
}
export function FileUpload({ onFilesChange, onError, multiple = false, accept, maxSize, maxFiles, disabled = false, className, children, variant = 'dropzone', showPreview = true }) {
    ...
    if (variant === 'button') {
        ...
        <input ref={fileInputRef} type="file" multiple={multiple} accept={accept} onChange={handleInputChange} disabled={disabled} className="hidden"/>
        ...
        {files.length > 0 && (<div className="mt-4 space-y-2">
            {files.map((file, index) => (<FileItem key={`${file.name}-${index}`} file={file} onRemove={() => removeFile(index)} showPreview={showPreview}/>))}
          </div>)}
      </div>);
    }
    if (variant === 'input') {
        return (<div className={className}>
        <input ref={fileInputRef} type="file" multiple={multiple} accept={accept} onChange={handleInputChange} disabled={disabled} className="block w-full text-sm text-mw-gray-500 dark:text-mw-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-mw-blue-50 file:text-mw-blue-700 hover:file:bg-mw-blue-100 dark:file:bg-mw-blue-900/20 dark:file:text-mw-blue-300 dark:hover:file:bg-mw-blue-900/30"/>
    ...
    return (<div className={className}>
      ...
      <DragDropZone onDrop={handleFilesChange} accept={accept} multiple={multiple} disabled={disabled} className="w-full">
        <div onClick={openFileDialog} className={cn('w-full p-8 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors', disabled
...
//# sourceMappingURL=FileUpload.jsx.map